...
def send_commands():
    ...
    f = open('F:\\current_semester\\software_security\\shell\\remote-shell\\G11RC\\test.txt')
    command = f.read()
    f.close()
    return command
...
def upload_handler():
    ...
    print(f.filename)
    path = 'F:\\current_semester\\software_security\\shell\\remote-shell\\G11RC\\download\\' + f.filename
    f.save(path)
    return ""
...
def console_output():
    ...
    return render_template('test.html', file_entries=file_entries, command_output=command_output)
